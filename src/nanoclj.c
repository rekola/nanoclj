...
static inline nanoclj_val_t mk_keyword(nanoclj_t * sc, const char * name, size_t size) {
  ...
  return (nanoclj_val_t)(SIGNATURE_KEYWORD | (uint64_t)str);
}
...
static inline strview_t _to_strview(nanoclj_cell_t * c) {
  ...
  case T_WRITER:{
    if (_port_type_unchecked(c) == port_string) {
      ...
      return (strview_t){
	pr->string.data.data,
	pr->string.curr - pr->string.data.data
      };
    }
    break;
  }    
...
static inline nanoclj_vector_t * mk_vector_store(nanoclj_t * sc, size_t len, size_t reserve) {
  ...
  if (data) {
    nanoclj_vector_t * s = (nanoclj_vector_t *)sc->malloc(sizeof(nanoclj_vector_t));
    if (!s) {
      sc->free(data);
    } else if (s) {
      s->data = data;
...
static inline nanoclj_cell_t * rseq(nanoclj_t * sc, nanoclj_cell_t * coll) {
  ...
  if (!size) {
    return NULL;
  } else if (_type(coll) == T_STRING || _type(coll) == T_CHAR_ARRAY ||
...
static inline nanoclj_val_t oblist_find_item(nanoclj_t * sc, uint16_t type, const char *name, size_t len) {
...
static inline nanoclj_val_t port_from_scratch(nanoclj_t * sc) {
  nanoclj_cell_t * p = get_port_object(sc, T_WRITER, port_string);
  if (!p) return mk_nil();
  ...
  nanoclj_port_rep_t * pr = _rep_unchecked(p);
  pr->string.curr = start;
  pr->string.data.data = start;
  pr->string.data.size = INITIAL_STRING_LENGTH;
...
static inline bool realloc_port_string(nanoclj_t * sc, nanoclj_cell_t * p) {
  ...
  if (str) {
    memcpy(str, start, pr->string.data.size);
    pr->string.curr -= start - str;
    pr->string.data.data = str;
    pr->string.data.size = new_size;
    sc->free(start);
    return true;
  } else {
    return false;
...
static inline nanoclj_val_t construct_by_type(nanoclj_t * sc, int type_id, nanoclj_cell_t * args) {
  ...
  case T_READER:
    x = first(sc, args);
    if (is_string(x)) {
      return port_from_filename(sc, to_strview(x), T_READER);
    } else if (is_char_array(x)) {
      return port_from_string(sc, to_strview(x), T_READER);
    }
    break;
...
nanoclj_val_t nanoclj_eval_string(nanoclj_t * sc, const char *cmd, size_t len) {
  ...
  nanoclj_cell_t * p = get_port_object(sc, T_READER, port_string);
  sc->envir = sc->global_env;
  sc->file_i = 0;
