...
static size_t get_vector_size(nanoclj_cell_t * vec) {
  return _is_small(vec) ? _sosize_unchecked(vec) : _size_unchecked(vec);
}

static void fill_vector(nanoclj_cell_t * vec, nanoclj_val_t elem) {
  size_t num = get_vector_size(vec);
...
static inline size_t strlength(nanoclj_val_t p) {
  if (!is_cell(p)) {
    ...
    return _strlength(decode_pointer(p));
  }
}
...
static inline nanoclj_cell_t * get_cell_x(nanoclj_t * sc, nanoclj_cell_t * a, nanoclj_cell_t * b) {
  if (sc->no_memory) {
    return &(sc->_sink);
  }
...
static inline nanoclj_cell_t * get_cell(nanoclj_t * sc, int flags, nanoclj_val_t a, nanoclj_val_t b, nanoclj_cell_t * meta) {
  nanoclj_cell_t * cell = get_cell_x(sc, is_cell(a) ? decode_pointer(a) : NULL, is_cell(b) ? decode_pointer(b) : NULL);
...
static inline nanoclj_val_t mk_ratio(nanoclj_t * sc, nanoclj_val_t num, nanoclj_val_t den) {  
  nanoclj_cell_t * x = get_cell_x(sc, is_cell(num) ? decode_pointer(num) : NULL, is_cell(den) ? decode_pointer(den) : NULL);
...
static inline nanoclj_cell_t * mk_seq(nanoclj_t * sc, nanoclj_cell_t * origin, size_t position) {
  nanoclj_cell_t * x = get_cell_x(sc, origin, NULL);
...
static inline nanoclj_cell_t * _get_vector_object(nanoclj_t * sc, int_fast16_t t, size_t offset, size_t size, nanoclj_vector_store_t * store) {
  nanoclj_cell_t * main_cell = get_cell_x(sc, NULL, NULL);
  if (sc->no_memory) {
    /* TODO: free the store */
    return &(sc->_sink);
  }
...
static inline bool equals(nanoclj_t * sc, nanoclj_val_t a0, nanoclj_val_t b0) {
  if (a0.as_long == b0.as_long) {
	...
	return _lvalue_unchecked(a) == _lvalue_unchecked(b);
      case T_VECTOR:
      case T_SORTED_SET:{
	size_t l = get_vector_size(a);
	if (l == get_vector_size(b)) {
...
static inline bool get_elem(nanoclj_t * sc, nanoclj_cell_t * coll, nanoclj_val_t key, nanoclj_val_t * result) {
  ...
  case T_VECTOR:
    if (is_number(key)) {
      ...
      if (index >= 0 && index < get_vector_size(coll)) {
  ...
  case T_ARRAYMAP:{
    size_t size = get_vector_size(coll);
...
static inline void new_frame_in_env(nanoclj_t * sc, nanoclj_val_t old_env) {
  ...
  sc->envir = mk_pointer(get_cell(sc, T_ENVIRONMENT, new_frame, old_env, NULL));
}

static inline nanoclj_cell_t * new_slot_spec_in_env(nanoclj_t * sc, nanoclj_val_t env,
...
static inline nanoclj_cell_t * find_slot_in_env(nanoclj_t * sc, nanoclj_val_t env, nanoclj_val_t hdl, bool all) {
  nanoclj_val_t x, y;
...
static inline nanoclj_val_t set_slot_in_env(nanoclj_t * sc, nanoclj_cell_t * slot, nanoclj_val_t state) {
  ...
  _cdr(slot) = state;
  nanoclj_cell_t * md = _metadata_unchecked(slot);
  if (md) {
    nanoclj_val_t watches0;
    if (get_elem(sc, md, sc->WATCHES, &watches0)) {
...
static inline int compare(nanoclj_val_t a, nanoclj_val_t b) {
  ...
  if (is_nil(a)) {
	...
	return 0;
      ...
      case T_VECTOR:
      case T_SORTED_SET:{
	size_t la = get_vector_size(a2), lb = get_vector_size(b2);
	if (la < lb) return -1;
	else if (la > lb) return +1;
	else {
...
static inline nanoclj_cell_t * copy_vector(nanoclj_t * sc, nanoclj_cell_t * vec) {
  ...
  if (_is_small(vec)) {
    ...
    nanoclj_vector_store_t * s = mk_vector_store(sc, len);
...
static inline nanoclj_cell_t * conj(nanoclj_t * sc, nanoclj_cell_t * coll, nanoclj_val_t new_value) {
  ...
  if (t == T_NIL || t == T_LIST || t == T_SEQ || t == T_LAZYSEQ) {
	...
	memcpy(_smalldata_unchecked(new_coll), data, old_size * sizeof(nanoclj_val_t));	
      } else {
...
static inline nanoclj_val_t intern(nanoclj_t * sc, nanoclj_val_t envir, nanoclj_val_t symbol, nanoclj_val_t value) {
...
static inline nanoclj_val_t intern_symbol(nanoclj_t * sc, nanoclj_val_t envir, nanoclj_val_t symbol) {
  nanoclj_cell_t * var = find_slot_in_env(sc, envir, symbol, false);
...
static inline nanoclj_val_t def_namespace_with_sym(nanoclj_t *sc, nanoclj_val_t sym) {
  ...
  nanoclj_val_t ns = mk_pointer(get_cell(sc, T_ENVIRONMENT, sc->EMPTY, sc->root_env, md));
  intern(sc, sc->global_env, sym, ns);
  return ns;
}
...
static inline nanoclj_val_t construct_by_type(nanoclj_t * sc, int type_id, nanoclj_val_t args0) {
  ...
  case T_DELAY:
    /* make closure. first is code. second is environment */
    return mk_pointer(get_cell(sc, type_id, cons(sc, sc->EMPTY, first(sc, args)), sc->envir, NULL));
...
static inline nanoclj_val_t opexe(nanoclj_t * sc, enum nanoclj_opcodes op) {
  ...
  case OP_REAL_EVAL:
	...
	if ((syn = syntaxnum(_car(code_cell))) != 0) {       /* SYNTAX */
      ...
      case T_VECTOR:{
	if (get_vector_size(code_cell) > 0) {
  ...
  case OP_LAMBDA1:{
    ...
    /* make closure. first is code. second is environment */
    s_return(sc, mk_pointer(get_cell(sc, T_CLOSURE, x, sc->envir, decode_pointer(name))));
  }
  ...
  case OP_LOOP:{                /* loop */   
    ...
    size_t n = get_vector_size(input_vec) / 2;
  ...
  case OP_LAZYSEQ:               /* lazy-seq */
    s_return(sc, mk_pointer(get_cell(sc, T_LAZYSEQ, cons(sc, sc->EMPTY, sc->code), sc->envir, NULL)));
...
int nanoclj_init_custom_alloc(nanoclj_t * sc, func_alloc malloc, func_dealloc free, func_realloc realloc) {  
  ...
  intern(sc, sc->global_env, def_symbol(sc, "root"), sc->root_env);
