...
static uint_fast16_t prim_type_extended(const nanoclj_val_t value) {
...
static uint_fast16_t prim_type(const nanoclj_val_t value) {
  uint64_t signature = value.as_long >> 48;
  switch (signature) {
  case SIGNATURE_CELL: return T_NIL;
  case SIGNATURE_BOOLEAN: return T_BOOLEAN;
...
static inline bool is_symbol(const nanoclj_val_t v) {
  return (v.as_long >> 48) == SIGNATURE_SYMBOL;
}

static inline bool is_keyword(const nanoclj_val_t v) {
...
static inline bool is_type(const nanoclj_val_t value) {
...
static inline nanoclj_val_t get_indexed_elem(nanoclj_cell_t * coll, int64_t ielem) {
  switch (_type(coll)) {
  case T_HASHMAP:
  case T_SORTED_HASHMAP:
    if (_is_small(coll)) {
...
static inline nanoclj_val_t get_indexed_key(nanoclj_cell_t * coll, int64_t ielem) {
  switch (_type(coll)) {
  case T_HASHMAP:
  case T_SORTED_HASHMAP:
    if (_is_small(coll)) {
...
static inline nanoclj_val_t get_indexed_value(nanoclj_cell_t * coll, int64_t ielem) {
  switch (_type(coll)) {
  case T_HASHMAP:
  case T_SORTED_HASHMAP:
    if (_is_small(coll)) {
...
static inline bool is_number(const nanoclj_val_t p) {
  switch (prim_type(p)) {
  case T_DOUBLE:
  case T_LONG:
    return true;
...
static size_t find_index(nanoclj_t * sc, nanoclj_cell_t * coll, nanoclj_val_t key) {
  ...
  case T_VAR:
    {
      long long index = to_long_w_def(key, -1);
...
static inline nanoclj_cell_t * new_slot_spec_in_env(nanoclj_t * sc, nanoclj_cell_t * env,
  ...
  if (is_vector(x)) {
    ...
    set_vector_elem(vec, location, mk_pointer(cons(sc, slot, decode_pointer(get_indexed_elem(vec, location)))));
  } else {
...
static inline nanoclj_cell_t * find_slot_in_env(nanoclj_t * sc, nanoclj_cell_t * env, nanoclj_val_t hdl, bool all) {  
  for (nanoclj_cell_t * x = env; x && x != &(sc->_EMPTY); x = decode_pointer(_cdr(x))) {
    ...
    if (_type(y) == T_VECTOR) {
      ...
      y = decode_pointer(get_indexed_elem(y, location));
    }
...
static inline nanoclj_cell_t * vector_conjoin(nanoclj_t * sc, nanoclj_cell_t * vec, nanoclj_val_t new_value) {
  ...
  if (_is_small(vec)) {
	...
	tensor_hash_mutate_set(tensor, h, i, v, mk_nil(), is_ordered, sc, is_ordered ? hashcmp : hasheq);
      }
      uint32_t h = is_ordered ? hashcmp(new_value, sc) : hasheq(new_value, sc);
      tensor_hash_mutate_set(tensor, h, old_size, new_value, mk_nil(), is_ordered, sc, is_ordered ? hashcmp : hasheq);
    } else {
...
static inline nanoclj_cell_t * associative_conjoin(nanoclj_t * sc, nanoclj_cell_t * coll, nanoclj_val_t key, nanoclj_val_t value) {
  ...
  if (j != NPOS) {
	...
	tensor_hash_mutate_set(coll->_collection.tensor, h0, 0, old_key, old_val, is_ordered, sc, is_ordered ? hashcmp : hasheq);
	tensor_hash_mutate_set(coll->_collection.tensor, h1, 1, key, value, is_ordered, sc, is_ordered ? hashcmp : hasheq);
      }
    } else {
