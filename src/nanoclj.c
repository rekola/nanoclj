...
static inline bool is_list(nanoclj_val_t value) {
  if (!is_cell(value)) return false;
  return _is_list(decode_pointer(value));
}
...
static inline int alloc_cellseg(nanoclj_t * sc, int n) {
  ...
  for (int k = 0; k < n; k++) {
    ...
    for (p = newp; p.as_long <= last.as_long; p = mk_pointer(decode_pointer(p) + 1)) {
      cell_type(p) = 0;
      cell_flags(p) = 0;
      set_cdr(p, decode_pointer(p) + 1);
      set_car(p, mk_emptylist());
    }
...
static inline void retain(nanoclj_t * sc, nanoclj_cell_t * recent) {
  ...
  if (holder) {
    ...
    _set_cdr(holder, decode_pointer(_car(&sc->sink)));
...
static inline nanoclj_val_t first(nanoclj_t * sc, nanoclj_cell_t * coll) {
  ...
  case T_ARRAYMAP:
    if (get_size(coll) > 0) {
      if (!_is_small(coll)) {
	nanoclj_tensor_t * tensor = coll->_collection.tensor;
	return mk_pointer(get_collection_object(sc, T_MAPENTRY, _offset_unchecked(coll) * tensor->ne[0], tensor->ne[0], tensor));
      } else {
  ...
  case T_VARMAP:
	...
	if (tensor_is_valid_offset(tensor, i)) {
	  return mk_pointer(get_collection_object(sc, t == T_VARMAP ? T_VAR : T_MAPENTRY, i * tensor->ne[0], tensor->ne[0], tensor));
	}
      } else if (t == T_VARMAP) {
...
static inline bool equals(nanoclj_t * sc, nanoclj_val_t a0, nanoclj_val_t b0) {
  ...
  if (is_map_type(t_a) && is_map_type(t_b)) {
	...
	return equals(sc, mk_pointer(_cdr(a)), mk_pointer(_cdr(b)));
      }
      break;
    case T_BIGINT:
    case T_BIGDECIMAL:
	  ...
	  case T_ENVIRONMENT:
	    {
	      ...
	      if (r) return r;
	      return compare(sc, mk_pointer(_cdr(a2)), mk_pointer(_cdr(b2)));
	    }
...
static inline nanoclj_cell_t * vector_conjoin(nanoclj_t * sc, nanoclj_cell_t * vec, nanoclj_val_t new_value) {
  ...
  if (_is_small(vec)) {
	...
	tensor = tensor_hash_mutate_set(tensor, h, i, v, mk_nil(), mk_nil(), is_ordered, sc, is_ordered ? hashcmp : hasheq);
      }
      uint32_t h = is_ordered ? hashcmp(new_value, sc) : hasheq(new_value, sc);
      new_vec->_collection.tensor = tensor_hash_mutate_set(tensor, h, old_size, new_value, mk_nil(), mk_nil(), is_ordered, sc, is_ordered ? hashcmp : hasheq);
    } else {
    ...
    bool is_ordered = t == T_SORTED_HASHSET;
    uint32_t h = is_ordered ? hashcmp(new_value, sc) : hasheq(new_value, sc);
    nanoclj_tensor_t * tensor = tensor_hash_mutate_set(vec->_collection.tensor, h, old_size, new_value, mk_nil(), mk_nil(), is_ordered, sc, is_ordered ? hashcmp : hasheq);
...
static inline nanoclj_cell_t * assoc_with_meta(nanoclj_t * sc, nanoclj_cell_t * coll, nanoclj_val_t key, nanoclj_val_t value, nanoclj_val_t meta) {
  ...
  if (j != NPOS) {
	...
	if (t == T_ARRAYMAP) {
	  ...
	  tensor = tensor_hash_mutate_set(tensor, h0, 0, old_key, old_val, old_meta, is_ordered, sc, is_ordered ? hashcmp : hasheq);
	  coll->_collection.tensor = tensor_hash_mutate_set(tensor, h1, 1, key, value, meta, is_ordered, sc, is_ordered ? hashcmp : hasheq);
	}
      }
    } else if (t == T_ARRAYMAP) {
      nanoclj_tensor_t * tensor = coll->_collection.tensor;
      ...
      nanoclj_tensor_t * tensor = tensor_hash_mutate_set(coll->_collection.tensor, h, old_size, key, value, meta, is_ordered, sc, is_ordered ? hashcmp : hasheq);
      coll = get_collection_object(sc, t, _offset_unchecked(coll), old_size + 1, tensor);
    }
  } else {
    nanoclj_throw(sc, mk_index_exception(sc, "Index out of bounds"));
...
static inline void new_var_in_ns(nanoclj_t * sc, nanoclj_cell_t * ns, nanoclj_val_t variable, nanoclj_val_t value, nanoclj_cell_t * meta) {
  nanoclj_cell_t * x = decode_pointer(_car(ns));
  /* TODO: ensure that x is a var-map */
  _set_car(ns, mk_pointer(assoc_with_meta(sc, x, variable, value, mk_pointer(meta))));
}

static inline valarrayview_t find_slot_in_env(nanoclj_t * sc, nanoclj_cell_t * env, nanoclj_val_t hdl, bool all) {
  for (nanoclj_cell_t * x = env; x; x = _cdr(x)) {
...
static inline nanoclj_cell_t * get_var_in_env(nanoclj_t * sc, nanoclj_cell_t * env, nanoclj_val_t hdl, bool all) {
  for (nanoclj_cell_t * x = env; x; x = _cdr(x)) {
	...
	if (_is_small(y)) {
	  ...
	  return get_collection_object(sc, T_VAR, idx * 3, 3, y->_collection.tensor);
	}
      }
    } else {
      for (; y; y = _cdr(y)) {
...
static inline void new_slot_in_env(nanoclj_t * sc, nanoclj_val_t variable, nanoclj_val_t value) {
  nanoclj_cell_t * x = decode_pointer(_car(sc->envir));
...
static inline bool set_var_in_ns(nanoclj_t * sc, nanoclj_cell_t * ns, nanoclj_val_t hdl, nanoclj_val_t state, nanoclj_cell_t * meta) {
  ...
  if (get_elem(sc, meta, sc->WATCHES, &watches0)) {
    nanoclj_val_t var = mk_pointer(get_collection_object(sc, T_VAR, idx * 3, 3, y->_collection.tensor));
...
static inline nanoclj_cell_t * reverse_in_place_with_symbol(nanoclj_t * sc, nanoclj_val_t term, nanoclj_cell_t * p) {
  nanoclj_cell_t * result = get_cell(sc, T_SYMBOL, 0, term, NULL, NULL);
  while (p) {
    nanoclj_cell_t * q = _cdr(p);
    _set_cdr(p, result);
    result = p;
...
static nanoclj_val_t get_out_port(nanoclj_t * sc) {
  return resolve_value(sc, sc->envir, sc->OUT_SYM);
}

static nanoclj_val_t get_in_port(nanoclj_t * sc) {
  return resolve_value(sc, sc->envir, sc->IN_SYM);
}
...
static inline nanoclj_val_t mk_object(nanoclj_t * sc, uint_fast16_t t, nanoclj_cell_t * args) {
  ...
  case T_TENSOR:
	...
	nanoclj_tensor_t * tensor = NULL;
	if (_is_small(c)) {
	  /* TODO: allow other types than i8 */
	  tensor = mk_tensor_1d(nanoclj_i8, size);
	  memcpy(tensor->data, get_const_ptr(c), size);
	} else {
	  tensor = get_tensor(c);
	}
	if (tensor) {
...
static inline int get_literal_fn_arity(nanoclj_t * sc, nanoclj_val_t body, bool * has_rest) {
  if (is_list(body)) {
    ...
    int n2 = get_literal_fn_arity(sc, mk_pointer(cdr(body)), &has_rest2);
...
static inline bool opexe(nanoclj_t * sc, enum nanoclj_opcode op) {
  ...
  case OP_EVAL:                /* main part of evaluation */
	...
	case T_LIST:
	  if ((syn = syntaxnum(_car(code_cell))) != 0) {       /* SYNTAX */
	    sc->code = mk_pointer(_cdr(code_cell));
	    s_goto(sc, syn);
  ...
  case OP_E1ARGS:              /* eval arguments */
    ...
    if (is_list(sc->code)) {    /* continue */
      s_save(sc, OP_E1ARGS, sc->args, mk_pointer(cdr(sc->code)));
      sc->code = car(sc->code);
      sc->args = NULL;
      s_goto(sc, OP_EVAL);
    } else {                    /* end */
  ...
  case OP_APPLY:               /* apply 'code' to 'args' */
	...
	if (!params) { /* lazy-seqs etc. do not have params */
	  ...
	  if (is_nil(x) || is_emptylist(x)) {
	    ...
	    new_slot_in_env(sc, car(x), mk_pointer(yy));
	  } else {
  ...
  case OP_DO:               /* do */
    ...
    if (cdr(sc->code)) {
      s_save(sc, OP_DO, NULL, mk_pointer(cdr(sc->code)));
    }
    sc->code = car(sc->code);
    s_goto(sc, OP_EVAL);
  ...
  case OP_LET2:                /* let */
    {
      new_frame_in_env(sc, sc->envir);
      nanoclj_cell_t * x = decode_pointer(is_symbol(car(sc->code)) ? cadr(sc->code) : car(sc->code));
      for (z = sc->args; z; x = _cdr(x), z = _cdr(z)) {
	new_slot_in_env(sc, _caar(x), _car(z));
      }
      sc->code = mk_pointer(cdr(sc->code));
      sc->args = NULL;
      s_goto(sc, OP_DO);
  ...
  case OP_AND0:                /* and */
    ...
    s_save(sc, OP_AND1, NULL, mk_pointer(cdr(sc->code)));
    sc->code = car(sc->code);
    s_goto(sc, OP_EVAL);
  ...
  case OP_OR0:                 /* or */
    ...
    s_save(sc, OP_OR1, NULL, mk_pointer(cdr(sc->code)));
    sc->code = car(sc->code);
    s_goto(sc, OP_EVAL);
  ...
  case OP_CONJ:             /* -conj */
	...
	Error_0(sc, "Value is not ISeqable");
      } else if (coll && _type(coll) == T_VECTOR && is_cell(arg1)) {
	nanoclj_cell_t * arg = decode_pointer(arg1);
	if (arg && _type(arg) == T_MAPENTRY) {
      ...
      if (!coll || _is_sequence(coll) || is_seqable_type(_type(coll))) {
  ...
  case OP_DEREF:
	...
	if (_type(c) == T_LAZYSEQ || _type(c) == T_DELAY) {
	  if (!_is_realized(c)) {
	    ...
	    sc->args = NULL;
	    s_goto(sc, OP_APPLY);
	  } else if (_type(c) == T_LAZYSEQ && is_nil(_car(c)) && !_cdr(c)) {
  ...
  case OP_RD_OLD_DOT:
	...
	if (token(sc, inport) != TOK_RPAREN) {
	  ...
	  _nesting_unchecked(inport)--;
	  s_return(sc, mk_pointer(reverse_in_place_with_symbol(sc, sc->value, sc->args)));
	}
      }
    }
    Error_0(sc, "Not a reader");
